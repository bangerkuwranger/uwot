		...
		if ('object' === typeof req.body && 'string' === typeof req.body.cmd) {
			...
			if ('object' === typeof req.uwot && 'object' === typeof req.uwot.cmdAst && null !== req.uwot.cmdAst)  {
				...
				if ('object' === typeof req.uwot.runtime && 'object' === typeof req.uwot.runtime.exes) {
					...
					req.uwot.runtime.executeCommands();
					// results must be an object
					if ('object' === typeof req.uwot.runtime.results) {
				
						// results.output must be a non-empty array
						if ('object' === typeof req.uwot.runtime.results.output && Array.isArray(req.uwot.runtime.results.output) && req.uwot.runtime.results.output.length > 0) {
				
							// process output lines from results.output to push to resObj.output.content array
							req.uwot.runtime.results.output.forEach(function(ol) {
						...
						if ('object' === typeof req.uwot.runtime.results.operations && Array.isArray(req.uwot.runtime.results.operations) && req.uwot.runtime.results.operations.length > 0) {
			
							// then loop through operations, building final operations in req.body.operations
							for (let i = 0; i < req.uwot.runtime.results.operations.length; i++) {
			
// 									resObj.output.content.push({tag:'br'}, JSON.stringify(req.uwot.runtime.results.operations[i]));
								// if element is an array, map its elements to an array of objects in req.body.operations
								if ('object' === typeof req.uwot.runtime.results.operations[i] && Array.isArray(req.uwot.runtime.results.operations[i])) {
				
									req.body.operations = req.uwot.runtime.results.operations[i].map((x) => ({ name: x.name, args: 'object' === typeof x.args ? x.args : [] }));
				
								}
								// if element is a non-null, non-array object, assign to req.body.operations and validate args
								else if ('object' === typeof req.uwot.runtime.results.operations[i] && null !== req.uwot.runtime.results.operations[i]) {
				
									req.body.operations = req.uwot.runtime.results.operations[i];
						...
						if ('object' === typeof req.uwot.runtime.results.cookies && null !== req.uwot.runtime.results.cookies) {
				
							resObj.cookies = req.uwot.runtime.results.cookies;
				
						}
						// if there are any user redirects, add them to resObj
						if ('string' === typeof req.uwot.runtime.results.redirect) {
				
							resObj.redirect = Url.parse(req.uwot.runtime.results.redirect, global.Uwot.Config.getConfigServerOrigin());
				
						}
						// if the runtime results return the current working directory for VFS
						// (which it always SHOULD)
						// add that to our resObj
						if ('string' === typeof req.uwot.runtime.results.cwd) {
				
							resObj.cwd = req.uwot.runtime.results.cwd;
