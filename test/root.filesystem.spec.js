...
// 				expect(filesystem.changeAllowed(testPath, testAllowed)).to.be.an.instanceof(Error).with.property('code').that.equals('ENOENT');
		...
		describe('readdirRecursive(pth, fType)', function() {
			...
			it('should only include directories if fType is "directory"', function() {
				...
				readDirStub.onCall(0).returns(['file', UWOT_HIDDEN_PERMISSIONS_FILENAME, 'dir', 'socket', 'symlink', 'fifo']);
				readDirStub.onCall(1).returns(['file']);
				expect(filesystem.readdirRecursive(testPath, 'directory')).to.deep.equal([path.join(testPath, 'dir')]);
			
			});
			it('should only include symbolic links if fType is "symlink"', function() {
				...
				readDirStub.onCall(0).returns(['file', UWOT_HIDDEN_PERMISSIONS_FILENAME, 'dir', 'socket', 'symlink', 'fifo']);
				readDirStub.onCall(1).returns(['file']);
				expect(filesystem.readdirRecursive(testPath, 'symlink')).to.deep.equal([path.join(testPath, 'symlink')]);
			
			});
			it('should only include sockets if fType is "socket"', function() {
				...
				readDirStub.onCall(0).returns(['file', UWOT_HIDDEN_PERMISSIONS_FILENAME, 'dir', 'socket', 'symlink', 'fifo']);
				readDirStub.onCall(1).returns(['file']);
				expect(filesystem.readdirRecursive(testPath, 'socket')).to.deep.equal([path.join(testPath, 'socket')]);
			
			});
			it('should only include FIFOs if fType is "fifo"', function() {
				...
				readDirStub.onCall(0).returns(['file', UWOT_HIDDEN_PERMISSIONS_FILENAME, 'dir', 'socket', 'symlink', 'fifo']);
				readDirStub.onCall(1).returns(['file']);
				expect(filesystem.readdirRecursive(testPath, 'fifo')).to.deep.equal([path.join(testPath, 'fifo')]);
			
			});
			it('should include any file type that is not a permissions file if fType is null', function() {
				...
				readDirStub.onCall(0).returns(['file', UWOT_HIDDEN_PERMISSIONS_FILENAME, 'dir', 'socket', 'symlink', 'fifo']);
				readDirStub.onCall(1).returns(['file']);
				expect(filesystem.readdirRecursive(testPath)).to.deep.equal([path.join(testPath, 'file'), path.join(testPath, 'dir'), path.join(testPath, 'dir/file'), path.join(testPath, 'socket'), path.join(testPath, 'symlink'), path.join(testPath, 'fifo')]);
			
			});
			it('should return an error if recursive read of a directory in starting path returns an error', function() {
				...
				var readDirStub = sinon.stub(filesystem, 'readDir');
				readDirStub.onCall(0).returns(['file', UWOT_HIDDEN_PERMISSIONS_FILENAME, 'dir', 'socket', 'symlink', 'fifo']);
				readDirStub.onCall(1).returns([null, 'file', UWOT_HIDDEN_PERMISSIONS_FILENAME, null]);
				expect(filesystem.readdirRecursive(testPath)).to.be.an.instanceof(Error).with.property('code').that.equals('EINVAL')
			
			});
			it('should push the results of a recursive directory read to the result array if the recursive read completes without error', function() {
				...
				statSyncStub.onCall(3).returns(testStatsDir);
				statSyncStub.onCall(4).returns(testStatsFile);
				statSyncStub.onCall(7).returns(testStatsSocket);
				statSyncStub.onCall(6).returns(testStatsSymlink);
				statSyncStub.onCall(5).returns(testStatsFifo);
				var readDirStub = sinon.stub(filesystem, 'readDir');
				readDirStub.onCall(0).returns(['file', UWOT_HIDDEN_PERMISSIONS_FILENAME, 'dir']);
				readDirStub.onCall(1).returns(['file', UWOT_HIDDEN_PERMISSIONS_FILENAME, 'symlink', 'fifo', 'socket']);
				expect(filesystem.readdirRecursive(testPath)).to.deep.equal([path.join(testPath, 'file'), path.join(testPath, 'dir'), path.join(testPath, 'dir/file'), path.join(testPath, 'dir/symlink'), path.join(testPath, 'dir/fifo'), path.join(testPath, 'dir/socket')]);
